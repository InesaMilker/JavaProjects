...
public class Demo1_StringBuilder extends BaseConsole{
    ...
    //https://en.wikipedia.org/wiki/List_of_Unicode_characters žiūr. punktą 30        
        ...
        addButton("pir\u25B2", action -> printPyramid(pyrHeight++, "\u25B2"));
        addButton("pir\u25CF", action -> printPyramid(pyrHeight++, "\u25CF"));
        addButton("reverse pir#", action -> printReversePyramid(ta1, revpyrHeight++, '#'));
    }
    private int pyrHeight = 1;
    private int revpyrHeight = 1;
...
// sudarykite pasirinktinai metodus apverstai ar pasuktai piramidei spausdinti -- Done
    private void printReversePyramid(TextArea ta, int h, char fillSym){
        
        final char emptySymbol = ':';
        ta.appendText("=== Reverse Piramide h = " + h + nL);
        
        StringBuilder emptySpace = new StringBuilder();
        emptySpace.append(emptySymbol);
        
        StringBuilder pyramid = new StringBuilder(new String(new char[2 * h - 1]).replace('\0', fillSym));
        
        for (int i = 0; i < h; i++) {         
            ta.appendText(emptySpace.toString() + pyramid.toString() + emptySpace + nL);
            if(pyramid.length() <= 1)
                break;
            emptySpace.append(emptySymbol);     
            pyramid.deleteCharAt(pyramid.length() - 1).deleteCharAt(pyramid.length() - 1);   
        }        
    }    
    @Override
